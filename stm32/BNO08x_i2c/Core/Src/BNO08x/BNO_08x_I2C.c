...
static uint8_t commandSequenceNumber __attribute__((unused)) = 0;
...
static BNO_CommandReq_t cmdRequest __attribute__((unused)) = {0};
...
// Public wrapper: send a Product ID request and wait for response
HAL_StatusTypeDef BNO_requestProductID(void) {
    resetHeader(REPORT_PRODUCT_ID_REQUEST);
    return waitForCommandResponse();
}
...
HAL_StatusTypeDef BNO_Init(void) {
	// Expect PS1=0, PS0=0, CS=1 and ADD=0 to be hardware-strapped by the board.
	// Do NOT toggle the RST pin here; the module is left powered and its pins
	// are managed externally per the board wiring. Keep BOOT high to avoid DFU.
	BNO_BOOT_On(); // leave BOOT high (DFU off)
	HAL_Delay(20);
	// Start microsecond timer
	start_timer();
	// Enable interrupt for the sensor (INT pin)
	HAL_NVIC_EnableIRQ(EXTI15_10_IRQn);
	// Query product ID; caller should configure features afterwards (Adafruit-like behavior)
	if (getID() == HAL_OK) {
		return HAL_OK;
	}
	return HAL_ERROR;
...
HAL_StatusTypeDef BNO_writeFRS(const uint16_t length, const uint16_t frsType) {
	...
	*(uint16_t *)&bufferIO[8] = frsType;
	...
	if(waitForCommandResponse() == HAL_OK) { 
		...
		uint8_t sendMoreData __attribute__((unused)) = 0;
		uint8_t completed __attribute__((unused)) = 0;
...
HAL_StatusTypeDef BNO_readFRS(const uint16_t frsType) {
	...
	*(uint16_t *)&bufferIO[8] = frsType;
	...
	if(waitForCommandResponse() == HAL_OK) { 
		...
		uint8_t Datalen __attribute__((unused)) = ((bufferIO[5] >> 4) & 0x0F);
