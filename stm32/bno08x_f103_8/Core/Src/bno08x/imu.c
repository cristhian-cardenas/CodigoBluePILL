...
/**
  ******************************************************************************
  * @file    imu.c
  * @brief   Driver BNO08x: orientación y aceleración lineal → g_systemState
  *
  * Sin integración ni odometría; solo lectura de sensores SH2.
  * 
...
/**
  * @brief  Callback llamado por SH2 cuando hay nuevos datos del sensor
  *
  * - Linear Acceleration: escribe g_systemState.lin_ax/y/z (m/s²).
  * - Game Rotation Vector: roll, pitch, heading → g_systemState.
  *
  * @param  cookie: Cookie pasado a sh2_setSensorCallback (no usado)
  * @param  event: Evento del sensor con los datos
  */
static void sensorCallback(void *cookie, sh2_SensorEvent_t *event)
{
    ...
    switch (value.sensorId)
    {
        ...
        {
            g_systemState.lin_ax = value.un.linearAcceleration.x;
            g_systemState.lin_ay = value.un.linearAcceleration.y;
            g_systemState.lin_az = value.un.linearAcceleration.z;
            break;
        }
