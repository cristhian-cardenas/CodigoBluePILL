...
// Flag para rastrear si el heading se ha inicializado al menos una vez
// Permite inicializar el heading la primera vez incluso si el accuracy es bajo
static volatile bool heading_initialized = false;

// Estado de calibración del magnetómetro (bits 1-0 de value.status)
// 0 = Unreliable, 1 = Accuracy low, 2 = Accuracy medium, 3 = Accuracy high
...
/**
  * @brief  Configura los sensores del IMU BNO08x
  * 
  * Configura los sensores Rotation Vector y Linear Acceleration con sus
...
static int configure_imu_sensors(void)
{
    ...
    // ===== CONFIGURAR ROTATION VECTOR (ORIENTACIÓN) =====
    // Configurar orientación absoluta (Rotation Vector) a 100 Hz
...
/**
  ...
  * - Rotation Vector: Calcula roll, pitch, heading y activa flag para envío por UART
  * 
  * @param  cookie: Cookie pasado a sh2_setSensorCallback (no usado)
  * @param  event: Evento del sensor con los datos
  */
static void sensorCallback(void *cookie, sh2_SensorEvent_t *event)
{
    ...
    switch (value.sensorId)
    {
        ...
        case SH2_ROTATION_VECTOR:
        {
            ...
            // Leer cuaternión del Rotation Vector
            ...
            // ===== VERIFICAR ACCURACY DEL MAGNETÓMETRO =====
            // El campo accuracy está en radianes y representa la incertidumbre del heading
            // Para yaw absoluto (0-360° real), necesitamos accuracy < 0.1745 rad (10°)
            // Si el accuracy es mayor, el BNO08x puede estar usando solo giroscopio/acelerómetro
            // lo que da yaw relativo, no absoluto
            float accuracy_rad = value.un.rotationVector.accuracy;
            
            // ===== VERIFICAR STATUS DEL SENSOR =====
            // Bits 1-0 del campo status indican la calidad de la lectura:
            // 0 = Unreliable, 1 = Accuracy low, 2 = Accuracy medium, 3 = Accuracy high
            // Para yaw absoluto confiable, necesitamos status >= 2 (medium o high)
            uint8_t accuracy_status = value.status & 0x03; // Extraer bits 1-0
            
            // Actualizar imu_status para diagnóstico externo
            imu_status = accuracy_status;
            
            // ===== CONDICIÓN PARA YAW ABSOLUTO =====
            // Solo usar el yaw si:
            // 1. El accuracy es finito y < 10° (0.1745 rad) - indica que el magnetómetro está calibrado
            // 2. El status es >= 2 (medium o high) - indica calidad suficiente
            // Si no se cumplen estas condiciones, el BNO08x puede estar usando solo
            // giroscopio/acelerómetro, dando yaw relativo en lugar de absoluto
            // Verificación adicional: asegurar que accuracy_rad sea finito (no NaN/Inf)
            bool accuracy_valid = (accuracy_rad == accuracy_rad) && (accuracy_rad >= 0.0f) && (accuracy_rad < 10.0f);
            bool yaw_absolute_valid = accuracy_valid && (accuracy_rad < 0.1745f) && (accuracy_status >= 2);
            ...
            // Si new_heading no es finito, no actualizar heading (mantener último valor válido)
            bool heading_finite = (new_heading == new_heading) && (new_heading >= 0.0f) && (new_heading <= 360.0f);
            
            // ===== ACTUALIZAR HEADING SOLO SI ES ABSOLUTO Y VÁLIDO =====
            // Si el accuracy y status son suficientes Y el heading es finito, actualizar el heading
            // Si no, mantener el último valor válido (el heading no se actualiza)
            // Esto asegura que solo usamos yaw absoluto cuando el magnetómetro está calibrado
            // 
            // EXCEPCIÓN: Si el heading nunca se ha inicializado, actualizarlo la primera vez
            // incluso si el accuracy es bajo, para evitar que permanezca en 0.0f
            // PERO solo si new_heading es finito (protección contra valores corruptos)
            if (heading_finite && (yaw_absolute_valid || !heading_initialized))
            {
                heading = new_heading;
                heading_initialized = true;
            }
            // Si yaw_absolute_valid es false y heading_initialized es true,
            // el heading mantiene su valor anterior
            // Esto evita usar yaw relativo cuando el magnetómetro no está calibrado
...
/**
  ...
  * 3. Configura los sensores Rotation Vector y Linear Acceleration
  * 
...
int imu_init(void)
{
    ...
    // ===== CONFIGURAR SENSORES =====
    // Configurar los sensores del IMU (Rotation Vector y Linear Acceleration)
...
void imu_service(void)
{
    ...
    if (print_flag)
    {
        ...
        // imu_status: estado de calibración del magnetómetro (0-3)
